...
proto_pcie_ring_state ProtoPcie::ring_state() const
{
    proto_pcie_ring_state out{};
    if (::ioctl(fd_, PROTO_PCIE_IOC_GET_RING_STATE, &out) != 0) {
        throw std::runtime_error("ioctl(GET_RING_STATE) failed: " + std::string(std::strerror(errno)));
    }
    return out;
}
...
void ProtoPcie::process_pending() const
{
    if (::ioctl(fd_, PROTO_PCIE_IOC_PROCESS_PENDING) != 0) {
        throw std::runtime_error("ioctl(PROCESS_PENDING) failed: " + std::string(std::strerror(errno)));
    }
}
